...
def scrape_single_product(SKU, driver, log_callback=None):
    ...
    try:
        ...
        log_error(
            f"[{SKU}] Timeout waiting for product or no-results message: {e}",
            log_callback=log_callback,
        )
        return None
    ...
    try:
        ...
        log_error(
            f"[{SKU}] Error checking no-results elements: {e}",
            log_callback=log_callback,
        )
    ...
    try:
        ...
        log_error(
            f"[{SKU}] Error clicking product link or loading detail page: {e}",
            log_callback=log_callback,
        )
        return None
    ...
    try:
        script_tag = soup.find(
            "script", {"type": "application/json", "id": re.compile(r"ProductJson")}
        )
        ...
        log_error(
            f"[{SKU}] Error parsing embedded product JSON: {e}",
            log_callback=log_callback,
        )
        product_json = None
    ...
    if product_json:
        ...
        try:
            ...
            product_info["Brand"] = (
                clean_string(brand_element.text)
        ...
        try:
            ...
            base_name = (
                clean_string(name_element.text)
        ...
        if not weight:
            try:
                ...
                log_error(
                    f"[{SKU}] Error extracting Weight (fallback): {e}",
                    log_callback=log_callback,
                )
                weight = "N/A"
        ...
        if missing:
            log_error(
                f"[{SKU}] Missing fields in extracted product_info: {', '.join(missing)}",
                log_callback=log_callback,
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        data = scrape_mazuri([SKU])
        if data and len(data) > 0:
            print("✅ Scraping successful!")
            print(data[0])
        else:
            print("❌ No product data found for SKU.")
    else:
        # Run default tests
        SKU_valid = "0057001"
        print(f"\nTesting with valid SKU: {SKU_valid}")
        data = scrape_mazuri([SKU_valid])
        if data and len(data) > 0:
            print("✅ Scraping successful!")
            print(data[0])
        else:
            print("❌ No product data found for valid SKU.")
