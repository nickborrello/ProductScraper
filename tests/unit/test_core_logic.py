...
def test_run_scraping_success(mock_callbacks, mock_scraper, mock_excel_validation, mock_pandas_read):
...
def test_run_scraping_empty_excel(mock_callbacks, mock_excel_validation, mock_pandas_read, mock_os_remove):
...
def test_run_db_refresh_xml_not_found(mock_callbacks):
    ...
    log_callback.assert_any_call("💡 Please download the XML from ShopSite first (Option 4 in CLI).")
