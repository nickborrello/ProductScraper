...
from util.scrape_display import display_success, display_error, display_warning
...
class GranularScraperTester:
    ...
    def test_single_field(self, scraper_func: callable, test_sku: str, field_name: str) -> FieldTestResult:
    ...
    def validate_field(self, product: dict, field_name: str, duration: float) -> FieldTestResult:
        ...
        if field_name == "SKU":
            if not value or str(value).strip() == "":
                return FieldTestResult(field_name, FieldTestStatus.FAIL, value, "SKU is empty", duration)
            if len(str(value).strip()) < 3:
                return FieldTestResult(field_name, FieldTestStatus.FAIL, value, "SKU too short", duration)
            ...
            if not value or str(value).strip() == "" or str(value).strip().upper() == "N/A":
                return FieldTestResult(field_name, FieldTestStatus.FAIL, value, "Name is empty or N/A", duration)
            if len(str(value).strip()) < 3:
                return FieldTestResult(field_name, FieldTestStatus.FAIL, value, "Name too short", duration)
            return FieldTestResult(field_name, FieldTestStatus.PASS, value, "", duration)

        elif field_name == "Brand":
            if value is None or str(value).strip() == "" or str(value).strip().upper() == "N/A":
                return FieldTestResult(field_name, FieldTestStatus.FAIL, value, "Brand is empty or N/A", duration)
            return FieldTestResult(field_name, FieldTestStatus.PASS, value, "", duration)

        elif field_name == "Weight":
            if value is None or str(value).strip() == "" or str(value).strip().upper() == "N/A":
                return FieldTestResult(field_name, FieldTestStatus.FAIL, value, "Weight is empty or N/A", duration)
            return FieldTestResult(field_name, FieldTestStatus.PASS, value, "", duration)

        elif field_name == "Image URLs":
            if not value or (isinstance(value, list) and len(value) == 0):
                return FieldTestResult(field_name, FieldTestStatus.FAIL, value, "No image URLs found", duration)
            if isinstance(value, list) and len(value) > 0:
                return FieldTestResult(field_name, FieldTestStatus.PASS, f"{len(value)} images", "", duration)
            return FieldTestResult(field_name, FieldTestStatus.FAIL, value, "Invalid image URLs format", duration)
