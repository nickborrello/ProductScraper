...
from PyQt6.QtWidgets import (
    ...
    QScrollArea, QFrame, QMessageBox, QGroupBox, QDialog, QDialogButtonBox
)
...
class ProductEditor(QMainWindow):
    ...
    def update_image_controls(self):
        ...
        self.right_pos_btn.setEnabled(has_imgs and len(self.current_images) > 1 and idx < len(self.current_images) - 1)
    ...
    def remove_image(self):
        if self.current_images and 0 <= self.current_image_index < len(self.current_images):
            ...
            if self.current_image_index >= len(self.current_images) and self.current_image_index > 0:
                self.current_image_index -= 1
            self.show_current_image()
...
def load_products_from_db(skus):
    ...
    try:
        ...
        for row in cursor.fetchall():
            sku, brand, name, weight, category, product_type, pages, special, images, disabled = row
