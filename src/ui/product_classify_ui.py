...
def get_facet_options_from_db(force_refresh=False):
    ...
    print(
        f"DEBUG: Querying database for category/product type options (cache {'expired' if _facet_cache['cache_timestamp'] else 'empty'} or force_refresh={force_refresh})"
    )
...
class ClassificationEditorWindow(QMainWindow):
    ...
    def setup_ui(self):
        self.setWindowTitle(
            f"Batch Classification Editor - Professional Edition - {len(self.products_list)} Products"
        )
    ...
    def normalize_selections(self, selections, available_options):
        ...
        if isinstance(selections, str):
            ...
            selections = [
                s.strip().title() for s in re.split(r"[|,]", selections) if s.strip()
            ]
            ...
            selections = [
                s.title() if isinstance(s, str) else str(s) for s in selections
            ]
        ...
        for product in products_list:
            if "_consolidated_data" in product:
                ...
                if "category_options" in consolidated:
                    additional_categories.update(
                        cat.title() for cat in consolidated["category_options"] if cat
                    )

                # Add product types from consolidated data
                if "product_type_options" in consolidated:
                    additional_product_types.update(
                        pt.title() for pt in consolidated["product_type_options"] if pt
                    )

                # Add pages from consolidated data
                if "product_on_pages_options" in consolidated:
                    additional_pages.update(
                        page.title()
                        for page in consolidated["product_on_pages_options"]
                        if page
                    )
