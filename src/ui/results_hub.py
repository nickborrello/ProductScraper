...
class ConsolidationWidget(QWidget):
    ...
    def _load_previous_selections(self):
        ...
        if self.results_hub:
            ...
            existing = next((p for p in self.results_hub.consolidated_products if p["sku"] == self.sku), None)
        ...
        if selected_scraper and selected_scraper in self.image_sources:
            ...
            for idx, img in enumerate(images):
                ...
                if img_url:
                    ...
                    """)
                    thumbnail_btn.clicked.connect(lambda checked, url=img_url: self.load_main_image(url))
                    ...
                    try:
                        ...
                        if not pixmap.isNull():
                            icon = QIcon(pixmap.scaled(76, 76, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation))
                    ...
                    self.thumbnail_layout.insertWidget(self.thumbnail_layout.count() - 1, thumb_container)
    ...
    def load_main_image(self, url: str):
        ...
        try:
            ...
            pixmap.loadFromData(data)
            if not pixmap.isNull():
                scaled = pixmap.scaled(480, 480, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
    ...
    def save_consolidated(self):
        ...
        if not self.selections:
            QMessageBox.warning(self, "No Selections", "Please select at least one field before saving.")
            return
...
class ResultsHub(QWidget):
    ...
    def load_session_history(self):
        ...
        files = sorted(results_dir.glob("scrape_session_*.json"), key=os.path.getmtime, reverse=True)
    ...
    def parse_session_data(self):
        ...
        if "session_id" in self.session_data and "results" in self.session_data:
            ...
            raw_results = {"Unknown": {item.get("SKU", "Unknown"): item for item in self.session_data}}
    ...
    def update_table(self):
        ...
        for item in self.consolidated_data:
            ...
            if self.search_term:
                ...
                for scraper_data in item["scrapers"].values():
                    search_text += f" {scraper_data.get('Name', '')} {scraper_data.get('Brand', '')}".lower()
        ...
        for row_idx, item in enumerate(filtered_data):
            ...
            # Status
            # Single-site products don't need consolidation
            if len(item["scrapers"]) == 1:
                status = "✅ Ready (Single Source)"
            elif self.is_consolidated(sku):
                status = "✅ Consolidated"
            else:
    ...
    def is_consolidated(self, sku: str) -> bool:
        """Check if a SKU has been consolidated."""
        return any(p["sku"] == sku for p in self.consolidated_products)
    ...
    def enter_consolidation_mode(self):
        """Enter consolidation mode - iterate through ALL unconsolidated multi-source products."""
        # Get all products with multiple sources that aren't consolidated yet
        products_to_consolidate = [
            item for item in self.consolidated_data 
            if len(item["scrapers"]) > 1 and not self.is_consolidated(item["sku"])
        ]
        
        if not products_to_consolidate:
            QMessageBox.information(self, "All Done", 
                                  "All multi-source products have been consolidated!")
            return
    ...
    def consolidate_product(self, sku: str):
        ...
        if len(product_data["scrapers"]) < 2:
            QMessageBox.information(self, "Single Source", 
                                  "This product only has data from one source. No consolidation needed.")
            return
    ...
    def on_consolidation_saved(self, consolidated_data: dict):
        ...
        existing_idx = next((i for i, p in enumerate(self.consolidated_products) if p["sku"] == consolidated_data["sku"]), None)
