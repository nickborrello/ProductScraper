...
def get_facet_options_from_db():
    ...
    try:
        ...
        for row in cursor.fetchall():
            if row[0]:
                categories.extend(
                    [cat.strip() for cat in str(row[0]).split("|") if cat.strip()]
                )
        ...
        for row in cursor.fetchall():
            if row[0]:
                product_types.extend(
                    [pt.strip() for pt in str(row[0]).split("|") if pt.strip()]
                )
        ...
        for row in cursor.fetchall():
            if row[0]:
                pages.extend(
                    [page.strip() for page in str(row[0]).split("|") if page.strip()]
                )
...
def query_cross_sell_candidates(
    category_filters, product_type_filters, page_filters, exclude_sku=None, limit=50
):
    """
    ...
    try:
        ...
        where_conditions.append(
            "(ProductDisabled IS NULL OR ProductDisabled != 'checked')"
        )
