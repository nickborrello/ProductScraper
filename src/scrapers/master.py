...
class ScrapingProgressTracker:
    ...
    def _display_progress(self, status_message=""):
        ...
        # Calculate ETA
        eta_str = ""
        if elapsed > 10 and overall_progress > 0:  # Only show ETA after 10 seconds and some progress
            ...
            try:
                ...
                if spec and spec.loader:
                    ...
                    if init_browser_func:
                        browser = init_browser_func(profile_suffix=unique_profile, headless=headless)
    ...
    for scraper_file in scraper_files:
        ...
        try:
            ...
            if scrape_func and callable(scrape_func):
                ...
                display_name = name_mapping.get(func_name, func_name.replace('scrape_', '').replace('_', ' ').title())
...
class ProductScraper:
    ...
    def run_scraper_tests(self, run_integration_tests=False):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                failed_scrapers = self._parse_test_failures(result.stdout + result.stderr)
                ...
                if failed_scrapers:
                    ...
                    for site, func in self.scraping_options.items():
                        ...
                        if module_name not in failed_scrapers:
                            ...
                            passing_headless[site] = self.headless_settings.get(site, True)
                    
                    self.scraping_options = passing_options
                    self.headless_settings = passing_headless
                    
                    print(f"✅ {len(passing_options)} scrapers passed tests and will be available")
                    ...
                    print("⚠️ Could not determine which scrapers failed, proceeding with all")
                    # Ensure scraping_options is initialized
                    if not hasattr(self, 'scraping_options'):
                        self.scraping_options, self.headless_settings = discover_scrapers()
                    return self.scraping_options
    ...
    def _parse_test_failures(self, test_output):
        ...
        for line in lines:
            ...
            # Also look for module names in error messages
                ...
                for module in ['orgill', 'bradley_caldwell', 'central_pet', 'coastal', 
    ...
    def get_scraping_order(self, completed_sites=None):
        ...
        while True:
            if not self.interactive:
                ...
                pending_sites = [site for site in all_sites if site not in completed_sites]
                if pending_sites:
            ...
            try:
                if ',' in order_input:
                    order_indices = [int(x.strip()) - 1 for x in order_input.split(',') if x.strip().isdigit()]
                else:
                    order_indices = [int(order_input) - 1] if order_input.isdigit() else []
                ...
                selected_sites = [sites_list[i] for i in order_indices if 0 <= i < len(sites_list)]
                if not selected_sites:
                    continue
                already_completed = [site for site in selected_sites if site in completed_sites]
    ...
    def get_existing_skus_from_database(self):
        ...
        try:
            ...
            try:
                ...
                existing_skus = {str(row[0]).strip() for row in cursor.fetchall() if row[0]}
    ...
    def save_incremental_results(self, new_row, source_site):
        ...
        try:
            ...
            if 'Price' in new_row:
                price_val = str(new_row['Price']).replace('$', '').replace('USD', '').strip()
                import re
            ...
            if os.path.exists(site_file_path):
                ...
                more_info_image_cols = [key for key in new_row.keys() if key.startswith('More Information Image')]
            ...
            image_count = 0
            for i in range(1, 6):
                if new_row.get(f'More Information Image {i}') and new_row.get(f'More Information Image {i}') != "none":
                    image_count += 1
        except Exception as e:
    ...
    def scrape(self):
        ...
        existing_skus_by_site = {site: self.load_existing_skus(site) for site in self.scraping_options.keys()}

        # Calculate initial SKUs to process per site (no filtering - process all)
        rows = list(df.iterrows())
        def is_valid_sku(sku_str):
            import re
            return bool(re.match(r'^\d{12}$', str(sku_str).strip()))  # Only accept SKUs that are exactly 12 digits
        for site in self.scraping_options.keys():
            if site == 'Excel':
                skus_for_site = [str(row['SKU']).strip() for _, row in rows]  # Process all SKUs regardless of existing
            else:
                skus_for_site = [str(row['SKU']).strip() for _, row in rows]  # Process all SKUs regardless of existing
            existing_count = len([str(row['SKU']).strip() for _, row in rows if str(row['SKU']).strip() in existing_skus_by_site.get(site, set())])
        ...
        while True:
            ...
            for site in scraping_sites:
                if site == 'Excel':
                    skus_to_process = [str(row['SKU']).strip() for _, row in rows]  # Process all SKUs regardless of existing
                else:
                    skus_to_process = [str(row['SKU']).strip() for _, row in rows]  # Process all SKUs regardless of existing
                existing_count = len([str(row['SKU']).strip() for _, row in rows if str(row['SKU']).strip() in current_existing_skus.get(site, set())])
            ...
            for site in scraping_sites:
                skus_for_this_site = [str(row['SKU']).strip() for _, row in rows]  # Process all SKUs for this site
                
                existing_count = len([str(row['SKU']).strip() for _, row in rows if str(row['SKU']).strip() in current_existing_skus.get(site, set())])
                site_results = self.process_site(site, skus_for_this_site, current_existing_skus, rows)
                if site_results:
        ...
        if all_found_skus:
            if self.interactive:
                remove_choice = input("\nRemove found SKUs from input file? (y/n): ").strip().lower()
            else:
            ...
            if remove_choice == 'y':
                try:
                    ...
                    remaining_df = original_df[~original_df['SKU'].astype(str).str.strip().isin(all_found_skus)]
        ...
        if scrape_all_mode:
            ...
            if unfound_skus:
                if self.interactive:
                    create_choice = input("\nCreate new products manually for these unfound SKUs? (y/n): ").strip().lower()
                else:
                    ...
                    try:
                        ...
                        for i, sku in enumerate(unfound_skus, 1):
                            ...
                            if result_path:
                                ...
                                cancel_all = input("Cancel creating remaining products? (y/n): ").strip().lower()
                                if cancel_all == 'y':
                                    break
    ...
    def process_site(self, site, skus_to_process, existing_skus_by_site, rows):
        ...
        try:
            ...
            for product_info in valid_products:
                ...
                input_row = next((r[1] for r in rows if str(r[1]['SKU']).strip() == sku), {})
        ...
        for sku, product_list in products_by_sku.items():
            ...
            for product_info, site, input_row in product_list:
                ...
                if product_cross_sell:
                    product_cross_sell_by_site[site] = product_cross_sell  # Single value per site
    ...
    def edit_consolidated_products(self, consolidated_products):
        ...
        for sku, product_data in consolidated_products.items():
            ...
            editor_product = {
                ...
                'Price': product_data.get('input_price', ''),  # Use original input price
    ...
    def save_final_products(self, final_products, rows):
        ...
        for product_info in final_products:
            ...
            for i, url in enumerate(image_urls[1:], start=1):
                if url:
                    local_image_path = f"{clean_brand(brand)}/{file_name.replace('.jpg', f'-{i}.jpg')}"
