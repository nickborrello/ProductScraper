# Task 2.4: Integrate the "Scraper Tests" Feature

**Assigned to:** Agent_GUI

## Objective
To enable running the scraper tests directly from the GUI for production safety checks.

## Expected Output
A functional "Run Tests" button that executes the `run_scraper_tests` function in a background thread and displays the results in the log.

## Guidance
This is another straightforward integration using the established worker pattern. The test SKU `035585499741` should be used as part of this process.

- Connect the `clicked` signal of the "Run Tests" button to a new slot method (e.g., `start_scraper_tests`).
