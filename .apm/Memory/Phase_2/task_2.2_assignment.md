# Task 2.2: Integrate the "Discontinued Check" Feature

**Assigned to:** Agent_GUI

## Objective
To provide a one-click method for running the discontinued product check from the GUI.

## Expected Output
A functional "Check Discontinued" button that runs the corresponding backend function in a background thread and displays its output in the log area.

## Guidance
This integration is simpler than the scraping one as it doesn't require a file input. The focus is on re-using the worker pattern.

- Connect the `clicked` signal of the "Check Discontinued" button to a new slot method (e.g., `start_discontinued_check`).
- In this method, instantiate the `Worker` thread, passing it the refactored `run_discontinued_check` function from `main.py`.
- Connect the worker's `finished` and `error` signals to slots that will display the output in the `QTextEdit` log area, then start the thread.
...
try:
    ...
    print(f"Error importing run_scraping: {e}")
    # Provide a dummy function if the import fails, so the GUI can still load.
...
class MainWindow(QMainWindow):
    ...
    def start_scraping(self):
        ...
        if file_path:
            ...
            self.worker = Worker(run_scraping, file_path)
    ...
    def handle_error(self, error_tuple):
        ...
        self.scraping_finished(is_error=True)

    def scraping_finished(self, is_error=False):
        if not is_error:
