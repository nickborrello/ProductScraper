# Task 2.1: Integrate File Selection and the "Start Scraping" Feature

**Assigned to:** Agent_GUI

## Objective
To implement the primary user workflow of selecting a file and starting the scraping process, with real-time feedback provided in the GUI.

## Expected Output
A functional "Start Scraping" button that uses a file dialog, triggers the scraping logic in a background thread, and displays progress and log output in the UI.

## Guidance
This task ties together all the foundational components built in Phase 1. Ensure the UI remains responsive and that all output is correctly piped to the log.

1.  **Connect Button Signal:** In the `MainWindow` class, connect the `clicked` signal of the "Start Scraping" `QPushButton` to a new slot method (e.g., `start_scraping`).
2.  **Implement File Dialog:** In the `start_scraping` method, use `QFileDialog.getOpenFileName` to open a native file dialog. Filter for appropriate file types (e.g., "Excel Files (*.xlsx *.xls);;Text Files (*.txt)"). If the user cancels the dialog, exit the method gracefully.
3.  **Instantiate Worker:** If a file is selected, create an instance of the `Worker` class. Pass the refactored `run_scraping` function from `main.py` and the selected file path to its constructor.
4.  **Connect Signals and Start:** Before starting the thread, connect the worker's `progress` and `finished` signals to slots in the `MainWindow` that will update the `QProgressBar` and log the results to the `QTextEdit`. Also connect the `error` signal. Finally, call the worker's `start()` method.
...
def run_scraping(file_path):
    ...
    if not is_valid:
        if "Permission denied" in message:
            print("❌ The file is open elsewhere. Please close it and retry.")
        else:
            print("⚠️ Please update the Excel file with required data.")
        return
    ...
    try:
        ...
        if df_check.empty:
            print(f"⚠️ Input file '{file_path}' is empty. Deleting file.")
            os.remove(file_path)
...
def run_discontinued_check(file_path):
    """Runs the discontinued product check."""
    if not DISCONTINUED_CHECKER_AVAILABLE:
        print("❌ DiscontinuedChecker module not available.")
        return
        ...
        for standard, variants in column_mapping.items():
            for variant in variants:
                if variant in df.columns and standard not in df.columns:
                    ...
                    print(f"📋 Mapped column {variant} -> {standard}")
