# Task 3.2: Add Unit Tests for Refactored Core Logic

**Assigned to:** Agent_Core

## Objective
To ensure the reliability and correctness of the decoupled backend logic by creating a suite of unit tests.

## Expected Output
A new test file (`test/test_core_logic.py`) containing `pytest` tests that validate the behavior of the functions refactored in Task 1.2.

## Guidance
- Use the `pytest` framework and `unittest.mock` to isolate functions from external dependencies like network requests or the database.
- Create a new file `test/test_core_logic.py`.
- Write test functions for `run_scraping`, `run_discontinued_check`, and the database refresh function.
