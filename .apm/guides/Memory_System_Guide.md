...
Memory duties are assigned to the *Manager Agent* - who maintains the system. Details on individual Memory Log files reside in guides/Memory_Log_Guide.md.
...
1. On phase entry, create `.apm/Memory/Phase_XX_<slug>/` if missing. For each task in the phase, create a **completely empty** Memory Log, following guides/Memory_Log_Guide.md:
    - `Task_Y_Z_<slug>.md`
